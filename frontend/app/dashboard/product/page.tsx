...
export default function ProductFeedPage() {
  ...
  const [newSIEntry, setSIEntry] = useState({
    StoreInvId: -1,
    PName: '',
    PCName: '',
    Brand: '', 
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (res.ok && json.success) {
          ...
          setError(json.error || 'Nem sikerült a bolt készletét betölteni.');
        }
      } catch (err) {
        setError('Nem lehet elérni a szervert.');
      } finally {
        setLoading(false);
      }
    };
...
const handleDeleteSIEntry = async (saleId: number) => {
...
// 1. THE BULLETPROOF SEARCH FILTER
  const filteredEntries = products.filter(product => {
    // Safely grab the text, or use an empty string if it's NULL in the database
    const PName = product?.PName || '';
    const PCName = product?.PCName || '';
    const Brand = product?.Brand || '';
    const Unit = product?.Unit || ''; 
    const Price = product?.Price || 0; 
    const Currency = product?.Currency || 'HUF'; 
    const Stock = product?.Stock || 0; 
    Description: '';
    const search = searchTerm.toLowerCase();

    return PName.toLowerCase().includes(search) ||
           PCName.toLowerCase().includes(search) ||
           Brand.toLowerCase().includes(search);
  });

const handleCreateSIEntry = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const token = localStorage.getItem('token');
      ...
      const res = await fetch('http://localhost:5000/api/StoreInventory', {
        ...
        body: JSON.stringify(newSIEntry)
      });
      ...
      alert("Nem lehet elérni a szervert.");
    }
  };
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-24 ">
          {filteredEntries.length === 0 ? (
            ...
            filteredEntries.map((product, index) => (
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-white leading-tight">
                      {product.ProductBrand} {sale.ProductName}
                    </h3>
                  </div>
